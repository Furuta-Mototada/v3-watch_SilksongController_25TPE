...
class DataCollectionDashboard:
    def __init__(
        self, udp_port=12345, output_dir="data/button_collected", skip_noise=False
    ):
    ...
    def draw_dashboard(self):
        ...
        print(
            f"{Colors.BOLD}{Colors.CYAN}         DATA COLLECTION DASHBOARD - Real-time Verification{Colors.RESET}"
        )
        ...
        print(
            f"{Colors.BOLD}Session:{Colors.RESET} {self.session_start.strftime('%Y-%m-%d %H:%M:%S')} "
            f"({session_duration})"
        )
        ...
        if not self.collection_started:
            ...
            print(
                f"  {Colors.DIM}(Press ENTER when both devices are connected){Colors.RESET}"
            )
        elif not self.baseline_noise_captured and not self.skip_noise:
            elapsed = (
                time.time() - self.noise_start_time if self.noise_start_time else 0
            )
            ...
            print(
                f"  ⏱  Time: {elapsed:.1f}s / {self.baseline_noise_duration}s "
        ...
        print(
            f"  Sensor: {self.latest_sensor_type:<20} | Freshness: {freshness} ({data_age:.1f}s ago)"
        )
        ...
        print(
            f"    X: {self.format_value(self.latest_accel['x'])}  "
        ...
        print(
            f"    X: {self.format_value(self.latest_gyro['x'])}  "
        ...
        print(
            f"    X: {self.format_value(self.latest_rotation['x'])}  "
        ...
        print(
            f"  WALK: {self.action_counts['walk']:>3}  IDLE: {self.action_counts['idle']:>3}  "
            f"PUNCH: {self.action_counts['punch']:>3}"
        )
        print(
            f"  JUMP: {self.action_counts['jump']:>3}  TURN_L: {self.action_counts['turn_left']:>3}  "
    ...
    def start(self):
        ...
        print(
            f"{Colors.BOLD}{Colors.CYAN}         DATA COLLECTION DASHBOARD - Starting Up{Colors.RESET}"
        )
        print(f"{Colors.CYAN}{'='*80}{Colors.RESET}\n")
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and self.phone_connected
                            and not ready_to_start
                        ):
                            ...
                            if not self.skip_noise:
                                print(
                                    f"📊 Capturing {self.baseline_noise_duration}s baseline noise..."
                                )
                            ...
                            self.dashboard_thread = threading.Thread(
                                target=self.dashboard_update_loop, daemon=True
                            )
        ...
        if msg_type == "label_event":
            ...
            if sensor_type == "rotation_vector":
                ...
                rot_x, rot_y, rot_z, rot_w = (
                    self.latest_rotation["x"],
                    self.latest_rotation["y"],
                    self.latest_rotation["z"],
                    self.latest_rotation["w"],
                )
            else:
            ...
            self.latest_timestamp = msg.get(
                "timestamp_ns", msg.get("timestamp", time.time() * 1e9)
            )
            ...
            if not self.baseline_noise_captured and self.noise_start_time:
                ...
                if elapsed <= self.baseline_noise_duration:
                    ...
                    if len(self.noise_buffer) > 0:
                        baseline_file = (
                            self.output_dir / f"baseline_noise_{int(time.time())}.csv"
                        )
    ...
    def handle_label_event(self, msg, addr):
        ...
        if event == "start":
            ...
            with self.lock:
                if (
                    not self.active_recording
                    or self.active_recording["action"] != action
                ):
                    return
    ...
    def segment_and_save_noise(self):
        ...
        locomotion_segments = self._segment_noise(
            self.noise_buffer, duration_sec=5.0, samples_per_sec=50
        )
        action_segments = self._segment_noise(
            self.noise_buffer, duration_sec=1.0, samples_per_sec=50
        )

        # Select 30 samples each
        selected_locomotion = random.sample(
            locomotion_segments, min(30, len(locomotion_segments))
        )
